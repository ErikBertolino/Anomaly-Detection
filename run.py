...
def main():
    ...
    dataset = dman.Dataset(
        normalize=FLAGS.datnorm,
        data=FLAGS.dataset,
        inlier=FLAGS.Inlier_Class,
        Inlier_size=FLAGS.Inlier_size,
        Outlier_size=FLAGS.Outlier_size,
    )

    if not (torch.cuda.is_available()):
        FLAGS.ngpu = 0
    device = torch.device(
        "cuda" if (torch.cuda.is_available() and FLAGS.ngpu > 0) else "cpu"
    )
    ...
    solver.training(
        ...
        Lgrad_weight=FLAGS.Lgrad_weight,
        split=FLAGS.Split,
    )
    # Validation
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="1 = MNIST, 2 = fMNIST, 3 = CIFAR10, 4 = CIFAR100",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="kernel size for constructing Neural Network",
    )
    parser.add_argument(
    ...
    parser.add_argument("--Inlier_Class", type=int, default=1, help="Inlier Class")
    parser.add_argument(
        "--Split", type=list, default=[0.5, 0.25, 0.25], help="Train/Valid/Test Split"
    )
    parser.add_argument(
        "--Lgrad_weight", type=float, default=1, help="Weight for Lgrad"
    )
    parser.add_argument(
        "--Inlier_size",
        type=int,
        default=1000,
        help="Amount of data points in the normal class",
    )
    parser.add_argument(
        "--Outlier_size",
        type=int,
        default=1000,
        help="Amount of data points in the abnormal class",
    )

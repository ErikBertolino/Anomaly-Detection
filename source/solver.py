...
def histogram(contents, savename=""):
    ...
    plt.text(
        x=xmax * 0.01,
        y=max(n1.max(), n2.max()),
        s="Histogram Intersection: %.3f" % (h_inter),
    )
...
def training(neuralnet, dataset, epochs, batch_size):
    ...
    for epoch in range(epochs):
        ...
        x_tr, x_tr_torch, y_tr, y_tr_torch, _ = dataset.next_train(
            batch_size=test_size, fix=True
        )  # Initial batch
        ...
        while True:
            ...
            x_tr, x_tr_torch, y_tr, y_tr_torch, terminator = dataset.next_train(
                batch_size
            )
            ...
            dis_x, features_real = neuralnet.discriminator(
                x_tr_torch.to(neuralnet.device)
            )
            dis_x_hat, features_fake = neuralnet.discriminator(
                x_hat.to(neuralnet.device)
            )

            l_tot, l_enc, l_con, l_adv = lfs.loss_ganomaly(
                z_code,
                z_code_hat,
                x_tr_torch,
                x_hat,
            ...
            for name, param in neuralnet.encoder.named_parameters():
                if name.endswith("weight"):
                    ...
                    target_grad = torch.autograd.grad(
                        recon_loss, param, create_graph=True
                    )[0]
                    ...
                    grad_loss = (
                        ...
                        * func.cosine_similarity(
                            target_grad.view(-1, 1),
                            ref_grad_enc[k].avg.view(-1, 1),
                            dim=0,
                        ).item()
                    )
        ...
        print(
            "Epoch [%d / %d] (%d iteration)  Enc:%.3f, Con:%.3f, Adv:%.3f, Grad:%3f, Total:%.3f"
    ...
    save_graph(
        contents=list_enc, xlabel="Iteration", ylabel="Enc Error", savename="l_enc"
    )
    save_graph(
        contents=list_con, xlabel="Iteration", ylabel="Con Error", savename="l_con"
    )
    save_graph(
        contents=list_adv, xlabel="Iteration", ylabel="Adv Error", savename="l_adv"
    )
    save_graph(
        contents=list_grad, xlabel="Iteration", ylabel="Adv Error", savename="l_grad"
    )
    save_graph(
        contents=list_tot, xlabel="Iteration", ylabel="Total Loss", savename="l_tot"
    )
...
def test(neuralnet, dataset):
    ...
    if len(param_paths) > 0:
        for idx_p, param_path in enumerate(param_paths):
            ...
            neuralnet.models[idx_p].load_state_dict(
                torch.load(PACK_PATH + "/runs/params-%d" % (idx_p))
            )
    ...
    while True:
        x_te, x_te_torch, y_te, y_te_torch, terminator = dataset.next_test(
            1
    ...
    while True:
        ...
        if outcheck:
            plt.imsave(
                os.path.join(
                    "test", "outbound", "%08d-%08d.png" % (testnum, int(score_anomaly))
                ),
                gray2rgb(gray=canvas),
            )
        else:
            plt.imsave(
                os.path.join(
                    "test", "inbound", "%08d-%08d.png" % (testnum, int(score_anomaly))
                ),
                gray2rgb(gray=canvas),
            )

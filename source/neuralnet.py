...
class NeuralNet(nn.Module):
    def __init__(
        self, height, width, channel, device, ngpu, ksize, z_dim, learning_rate=1e-3
    ):
        ...
        for idx_m, model in enumerate(self.models):
            if (self.device.type == "cuda") and (self.models[idx_m].ngpu > 0):
                self.models[idx_m] = nn.DataParallel(
                    self.models[idx_m], list(range(self.models[idx_m].ngpu))
                )
...
class Decoder(nn.Module):
    def __init__(self, height, width, channel, ngpu, ksize, z_dim):
        ...
        self.de_dense = nn.Sequential(
            ...
            nn.BatchNorm1d(
                (self.height // (2 ** 2)) * (self.width // (2 ** 2)) * self.channel * 64
            ),
            nn.ELU(),
        )
    ...
    def forward(self, input):
        ...
        denseout_res = denseout.view(
            denseout.size(0), 64, (self.height // (2 ** 2)), (self.height // (2 ** 2))
        )
